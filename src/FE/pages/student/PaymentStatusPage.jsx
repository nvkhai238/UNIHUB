    ...
    <section className="mx-auto max-w-2xl px-4 py-8">
      ...
      {message && (
        <div className={`mb-4 rounded-lg border p-4 text-sm ${
          message.includes('thành công') || message.includes('Đã đưa')
              ...
              {countdown !== null && (
                ...
                >
                  ...
                  <p className={`mt-1 font-mono text-3xl font-bold ${
                    countdown <= 60 ? 'animate-pulse text-red-700' : countdown <= 300 ? 'text-amber-700' : 'text-blue-700'
                  }`}
                  >
                    {String(Math.floor(countdown / 60)).padStart(2, '0')}:{String(countdown % 60).padStart(2, '0')}
                  </p>
                  {countdown <= 60 && countdown > 0 && (
                    <p className="mt-1 text-xs font-semibold text-red-600">Sắp hết thời gian thanh toán!</p>
                  )}
                  {countdown === 0 && (
                    <p className="mt-1 text-xs font-bold text-red-700">Đã hết thời gian thanh toán. Giao dịch sẽ bị hủy.</p>
                  )}
                </div>
              )}
...
function InfoRow({ label, value, mono }) {
  return (
    <div className="flex justify-between gap-4 text-sm">
      ...
      <span className={`font-medium text-gray-950 ${mono ? 'break-all font-mono text-xs' : ''}`}>
        {value || '—'}
      </span>
    </div>
  );
